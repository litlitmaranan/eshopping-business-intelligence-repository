...
<body>
 <div class="content">
		...
		<div id="main">
			...
			<div class="left">
				<h2><font color="blue">${customer}</font> <font color="red">${guest}</font>
		<font color="red">${successfulMessage}</font> <font color="red">${failureMessage}</font></font></h2>
				<form:form method="GET" action="/Eshopping/backToMenuShop">
						 <input type="hidden" value="${customer}" name="customer" class="login login-submit"/> 
						 <input value="<<back" name="backPageShop" type="submit" class="login login-submit"/> 
				</form:form>
				<p>
				<form:form method="GET" action="/Eshopping/goToCart">
						<input type="hidden" id="username" name="customer" value="${customer}" class="login login-submit"/>
						<input type="submit" value="Show Cart" name="showCart" class="login login-submit"/>
				</form:form>
		...
		<tbody>
			<c:forEach var="product" items="${productlist}">
				...
				<td><form:form method="GET"
						action="/Eshopping/temporaryStorage">
						...
						<input type="submit" value="Add to Cart" name="addToCart" class="login login-submit"/>
					</form:form></td>
			</c:forEach>
		</tbody>
	</table>
